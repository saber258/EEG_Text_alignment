...
from dataset_new import EEGDataset, TextDataset, Whole, Text_EEGDataset
from config import *
...
def train_epoch(train_loader1, device, model, optimizer, total_num, total_num2):
    ...
    for batch in tqdm(train_loader1, mininterval=100, desc='- (Training)  ', leave=False): 
      ...
      loss2, n_correct2, cnt2 = cal_loss(pred2, label1, device)
      # print(cnt2)
    ...
    return train_loss, train_acc, cm, cm2 #cnt_per_class, cm
...
def test_epoch(valid_loader, device, model, total_num, total_num2):
    ...
    np.savetxt(f'baselines/text_eeg_CE/{emotion}_{model_name_base}_all_pred.txt',all_pred)
    np.savetxt(f'baselines/text_eeg_CE/{emotion}_{model_name_base}_all_pred2.txt',all_pred2)
    np.savetxt(f'baselines/text_eeg_CE/{emotion}_{model_name_base}_all_label.txt', all_labels)
    all_pred = np.array(all_pred)
    all_pred2 = np.array(all_pred2)
    plot_roc(all_labels,all_pred)
    plot_roc(all_labels, all_pred2)
    cm1 = confusion_matrix(all_labels, all_res)
    cm2 = confusion_matrix(all_labels, all_res2)
    print("test_cm:", cm1)
    
    acc_SP, pre_i, rec_i, F1_i = cal_statistic(cm1)
    acc_SP2, pre_i2, rec_i2, F1_i2 = cal_statistic(cm2)
    print('acc_SP is : {acc_SP}'.format(acc_SP=acc_SP))
    print('pre_i is : {pre_i}'.format(pre_i=pre_i))
    print('rec_i is : {rec_i}'.format(rec_i=rec_i))
...
if __name__ == '__main__':
    model_name_base = 'baseline_eeg_plus_textCE'
    model_name = f'{emotion}_baseline_eeg_plus_textCE.chkpt'
    ...
    for r in range(1):
        ...
        optimizer = ScheduledOptim(
            Adam(filter(lambda x: x.requires_grad, model.parameters()),
                 betas=(0.9, 0.98), eps=1e-4, weight_decay = 1e-5), d_model, warm_steps)
        ...
        for epoch_i in range(epoch):
            print('[ Epoch', epoch_i, ']')
            start = time.time()
            train_loss, train_acc, train_cm, train_cm2 = train_epoch(train_loader_text_eeg, device, model, optimizer, train_text_eeg.__len__(), train_text_eeg.__len__())
      

            train_accs.append(train_acc)
            train_losses.append(train_loss)
            start = time.time()
            valid_loss, valid_acc, valid_cm, valid_cm2, eva_indi = eval_epoch(valid_loader_text_eeg, device, model, val_text_eeg.__len__(), val_text_eeg.__len__())
            ...
            if eva_indi >= max(eva_indis):
                torch.save(checkpoint, 'baselines/text_eeg_CE/'+str(r)+model_name)
        ...
        plt.savefig(f'baselines/text_eeg_CE/{emotion}_{model_name_base}results_%s_loss.png'%r)
        ...
        plt.savefig(f'baselines/text_eeg_CE/{emotion}_{model_name_base}results_%s_acc.png'%r)
        

        test_model_name = 'baselines/text_eeg_CE/'+ str(r) + model_name
        model = Transformer(device=device, d_feature=32, d_model=d_model, d_inner=d_inner,
