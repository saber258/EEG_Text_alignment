...
def eval_epoch(valid_loader1, device, model, total_num, total_num2):
    ...
    return valid_loss, valid_acc, cm,sum(rec_i[1:]) * 0.6 + sum(pre_i[1:]) * 0.4, all_pred, all_pred2, all_labels
...
def test_epoch(valid_loader, device, model, total_num, total_num2):
    ...
    np.savetxt(f'baselines/cossim/{emotion}_{model_name_base}_all_pred.txt',all_pred)

    np.savetxt(f'baselines/cossim/{emotion}_{model_name_base}_all_label.txt', all_labels)
...
if __name__ == '__main__':
    model_name_base = 'baseline_cossim_trans'
    model_name = f'{emotion}_baseline_cossim_trans.chkpt'
    ...
    for r in range(1):
        ...
        for epoch_i in range(epoch):
            ...
            start = time.time()
            valid_loss, valid_acc, valid_cm, eva_indi, all_pred_val, all_pred2_val, all_label_val = eval_epoch(valid_loader_text_eeg, device, model, val_text_eeg.__len__(), val_text_eeg.__len__())
            ...
            if eva_indi >= max(eva_indis):
                torch.save(checkpoint, 'baselines/cossim/'+str(r)+model_name)
        ...
        np.savetxt(f'baselines/cossim/{emotion}_{model_name_base}_all_pred_val.txt',pred_val)
        np.savetxt(f'baselines/cossim/{emotion}_{model_name_base}_all_pred2_val.txt',pred2_val)
        np.savetxt(f'baselines/cossim/{emotion}_{model_name_base}_all_label_val.txt', label_val)
        print('ALL DONE')               
        ...
        plt.savefig(f'baselines/cossim/{emotion}_{model_name_base}results_%s_loss.png'%r)
        ...
        plt.savefig(f'baselines/cossim/{emotion}_{model_name_base}results_%s_acc.png'%r)
        

        test_model_name = 'baselines/cossim/'+str(r) + model_name
