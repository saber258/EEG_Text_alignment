...
if __name__ == '__main__':
    ...
    model = Transformer(device=device, d_feature=768, d_model=d_model, d_inner=d_inner,
    ...
    for epoch_i in range(epoch):
        print('[ Epoch', epoch_i, ']')
        start = time.time()
        train_loss, train_acc, train_cm, all_pred, all_label  = train_epoch(train_loader_text_eeg, device, model, optimizer, train_text_eeg.__len__())
    ...
    new_df.to_csv('baselines/text/text_acc_loss.csv')


    np.savetxt(f'baselines/text/{emotion}_{model_name_base}_all_pred_train.txt',train_pred)
    np.savetxt(f'baselines/text/{emotion}_{model_name_base}_all_label_train.txt', train_label)
