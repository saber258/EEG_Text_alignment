...
if __name__ == '__main__':
    ...
    optimizer = ScheduledOptim(
        Adam(filter(lambda x: x.requires_grad, model.parameters()),
              betas=(0.9, 0.98), eps=1e-4, lr = 1e-6, weight_decay=1e-2), d_model, warm_steps)
    ...
    new_df.to_csv('baselines/fusion_cossim/fusion_cs_acc_loss.csv')

        
    np.savetxt(f'baselines/fusion_cossim/{emotion}_{model_name_base}_all_pred_train.txt',train_pred)
    np.savetxt(f'baselines/fusion_cossim/{emotion}_{model_name_base}_all_label_train.txt', train_label)
