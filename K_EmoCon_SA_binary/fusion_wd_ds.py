...
import numpy as np
import pandas as pd
from model_new import Transformer, Transformer2, DeepCCA, CAM
...
if __name__ == '__main__':
    model_name_base = 'baseline_fusion_wd_eeg_trans'
    model_name = f'{emotion}_baseline_fusion_wd_eeg_trans.chkpt'
    ...
    for epoch_i in range(epoch):
        print('[ Epoch', epoch_i, ']')
        start = time.time()
        train_loss, train_acc, train_cm, all_pred_train, all_label = train_epoch(train_loader_text_eeg, device, model, optimizer, train_text_eeg.__len__(), train_text_eeg.__len__())
  
        label_train.extend(all_label)
    ...
    np.savetxt(f'baselines/fusion_wd_ds/{emotion}_{model_name_base}_all_pred_train.txt',pred_train)
    np.savetxt(f'baselines/fusion_wd_ds/{emotion}_{model_name_base}_all_label_train.txt',label_train)
