...
def test_epoch(valid_loader, device, model, total_num, total_num2):
    ...
    cm = confusion_matrix(all_labels, all_res)
    X_labels = ['award', 'control', 'education', 'employer', 'founder', 'job title', 'nationality', 'political affiliation', 'visited', 'wife']
    sns.heatmap(cm, annot = True, cmap = 'YlGnBu',xticklabels = X_labels, yticklabels = X_labels)
    plt.savefig('confusion_matrix_RD.png', bbox_inches="tight")
    ...
    for r in range(1):
        ...
        optimizer = ScheduledOptim(
            Adam(filter(lambda x: x.requires_grad, model.parameters()),
                 betas=(0.9, 0.98), eps=1e-4 ,lr = 1e-2), d_model, warm_steps)
