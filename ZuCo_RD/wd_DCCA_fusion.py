...
def cal_loss(pred1, pred2, label1, out, device):
    ...
    loss3 = wasserstein_distance(pred1.cpu().detach().numpy().flatten(), 
    pred2.cpu().detach().numpy().flatten())

    loss3 = torch.tensor(loss3, requires_grad=True)
...
if __name__ == '__main__':
    model_name_base = 'baseline_DCCA_wd_trans'
    model_name = f'{emotion}_baseline_DCCA_Wd_trans.chkpt'
    ...
    df_train_text = df_train[[emotion, '0']]
    ...
    model = DeepCCA_fusion(model3, outdim_size = outdim_size, use_all_singular_values = False, d_feature = 20, d_model = d_model, d_inner = d_inner,
        n_layers=num_layers, n_head = num_heads, d_k=64, d_v=64, dropout = 0.1,
        class_num=10, device=torch.device('cuda'))
    ...
    new_df.to_csv('baselines/DCCA_fusion/dcca_wd_acc_loss.csv')
