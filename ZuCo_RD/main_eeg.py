...
def test_epoch(valid_loader, device, model, total_num):
    ...
    np.savetxt(f'baselines/eeg/{model_name_base}_all_pred.txt',all_pred)
    np.savetxt(f'baselines/eeg/{model_name_base}_all_label.txt', all_labels)
...
if __name__ == '__main__':
    ...
    model_name = f'relation_baseline_onlyeeg.chkpt'
    ...
    model = Transformer(device=device, d_feature=833, d_model=d_model, d_inner=d_inner,
    ...
    new_df.to_csv('baselines/eeg/eeg_acc_loss.csv')
    np.savetxt(f'baselines/eeg/{model_name_base}_all_pred_val.txt',valid_pred)
    np.savetxt(f'baselines/eeg/{model_name_base}_all_label_val.txt', valid_label)
    print('ALL DONE')               
    ...
    plt.savefig(f'baselines/eeg/{model_name_base}results_%s_loss.png'%r)
    ...
    plt.savefig(f'baselines/eeg/{model_name_base}results_%s_acc.png'%r)
